...
#ifdef TTDBP
		if (flags[10])
				    mult_darray3d(mn_ttdbp, NZ, NXMEM, NYMEM, frac);
#endif
		/*-----------------------------------
		 *
...
#ifdef TTDBP
		if (flags[9])
		    set_darray3d_zero(mn_ttdbp, NZ, NXMEM, NYMEM);
...
#endif
#ifdef TTDBP
    copy_darray3d(mn_ttdbp, ttdbp, NZ, NXMEM, NYMEM);
#endif
    for (k = 0; k < NZ; k++)
	{
	for (i = 0; i < NXMEM; i++)
