...
if(mn_jprod == NULL) {
	fprintf(stderr,"not enough memory for mn_jprod!\n");
	return 1;
}
mn_jremin = alloc3d(NZ,NXMEM,NYMEM);
if(mn_jremin == NULL) {
	fprintf(stderr,"not enough memory for mn_jremin!\n");
	return 1;
}
mn_jremdop = alloc3d(NZ,NXMEM,NYMEM);
if(mn_jremdop == NULL) {
	fprintf(stderr,"not enough memory for mn_jremdop!\n");
	return 1;
}
# ifdef PROGNOSTIC
