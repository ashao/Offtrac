...
int main(void)
{
	...
	int imon, inxt, ilst;
#ifdef ADJOINT
	int adjmon[12]; // Adjoint month list
#endif
# ifndef WRTTS
int itts; /* tracer time step counter */
# endif
...
#ifdef ADJOINT // Redefine order of months for adjoint
...
#ifdef ADJOINT
printf("Allocating arrays for adjoint\n");
allocate_adjoint( );
#endif
...
#endif
#ifdef ADJOINT
if (flags[10]) 	set_darray2d_zero(mn_adjoint, NXMEM, NYMEM);
/*
if (flags[11])
	mn_ttdinv = 0.0;*/
#endif
...
for (cmon = inmon; cmon < inmon + tmon; cmon++)
{
	...
	currtime = dyr + dmon[imon]/365; //ashao: advance currenttime

#ifdef ADJOINT
	ismon = adjmon[ismon];
	imon = adjmon[imon];
	snxt = (double) ((int) (ismon - 1) % NMONTHS);
	isnxt = (int) (snxt + 0.00001);
#endif
...
//DT
		...
		for (i = 0; i < NXMEM; i++)
		{
			for (j = 0; j < NYMEM; j++)
			{
				mn_adjoint[i][j] = adjoint_tracer[i][j];
			}
		}

		*mn_adjointinv = *adjointinv;
...
//			mult_darray2d(mn_adjoint, NXMEM, NYMEM, frac);
...
#ifdef ADJOINT
var[10] = &mn_adjoint[0][0];
// var[11] = &mn_adjointinv;
#endif
