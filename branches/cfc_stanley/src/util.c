...
double lin_interp(double X[], double V[], double Xq, int npts) {
	int i;
	int idx1,idx2;
	...
	// Find index in vector X that's closest to the query value Xq;
	dist_prev = pow(Xq-X[0],2);

	for (i=0;i<npts;i++) {
		dist_curr = pow(Xq-X[i],2);
	...
	// Handle the case if outside the range of input vector
	if (idx2<0 || idx2>=npts) {
		printf("WARNING: Requested interpolation outside the range of the "
				"input vector\n");
		Vq = V[idx1];
		return Vq;
	}
	// Handle the case where the interpolated query is a value in X
	if ( pow(X[idx1] - Xq,2)<eps ) {
		Vq = V[idx1];
		return Vq;
	}
	// Perform linear interpolation to get the appropriate value
	Vq = (V[idx2]-V[idx1]) / (X[idx2]-X[idx1]) * (Xq-X[idx1])+V[idx1];
	return Vq;
