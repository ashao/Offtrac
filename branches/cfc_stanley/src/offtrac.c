...
struct vardesc vars[NOVARS] =
{
		...
		{
				"mn_cfc11_sat",
				"CFC-11 surface saturation concentration",
				'h',
				'1',
				's',
				"mol/L",
		...
		{
				"mn_cfc12_sat",
				"CFC-12 surface saturation concentration",
				'h',
				'1',
				's',
				"mol/L",
		...
		{
				"mn_sf6_sat",
				"SF6 surface saturation concentration",
				'h',
				'1',
				's',
				"mol/L",
...
#ifdef CFC
Gas_transient_t cfc11_props,cfc12_props,sf6_props;
double ***mn_cfc11;
double ***mn_cfc12;
double ***mn_sf6;
double ***cfc11;
double ***cfc12;
double ***sf6;
double mn_cfc11_sat[NXMEM][NYMEM];
double mn_cfc12_sat[NXMEM][NYMEM];
double mn_sf6_sat[NXMEM][NYMEM];
...
#ifdef CFC
	flags[10] = 1; rflags[10]=1; // CFC-11 tracer
	flags[11] = 1; rflags[11]=1; // CFC-12 tracer
	flags[12] = 1; rflags[12]=1; // SF6 tracer
	flags[13] = 1; rflags[13]=1;
	flags[14] = 1; rflags[14]=1;
	flags[15] = 1; rflags[15]=1;
#endif
...
if (usehindcast) {
	...
	if ( (currtime >= BEGHIND) && (currtime < (ENDHIND+1) ) ) {
		...
		read_gas_exchange_fields(directory, hindindex,"hind");
	}
	else {
	read_h(isnxt, hend, "clim");
	read_gas_exchange_fields(directory, ismon,"clim");
	}
}
else {
	read_h(isnxt, hend, "clim");
	...
	for (itts = 1; itts <= NTSTEP; itts++)
	{
		...
		if (usehindcast) {
			if ( ( cmon >= starthindindex) && ( hindindex <= (numhindmonths-1) )) {
				...
				read_uvw(isnxt,"clim");
				read_h(isnxt, hend,"clim");
				read_gas_exchange_fields(directory, isnxt,"clim");
			}
		}
		else {
			printf("Reading in UVW from climatology\n");
...
#ifdef CFC
		if (flags[10])
			add_darray3d(mn_cfc11,cfc11,NZ,NXMEM,NYMEM);
		if (flags[11])
			add_darray3d(mn_cfc12,cfc12,NZ,NXMEM,NYMEM);
		if (flags[12])
			add_darray3d(mn_sf6,sf6,NZ,NXMEM,NYMEM);
#endif
...
#endif
#ifdef CFC
copy_darray3d(mn_cfc11,cfc11,NZ,NXMEM,NYMEM);
copy_darray3d(mn_cfc12,cfc12,NZ,NXMEM,NYMEM);
copy_darray3d(mn_sf6,sf6,NZ,NXMEM,NYMEM);
#endif
...
for (m = 0; m < NOVARS; m++)
	if (rflags[m])
	{
		err = write_field(cdfid, fn, vars[m], varinfo[varmap[m]], (size_t) 0,
				(double *) var[m]);
		if (err == -1)
			printf("Error writing %s.\n", vars[m].name);
	}
...
void alloc_fields(void)
{
	...
	var[12] = &mn_sf6[0][0][0];
	var[13] = &mn_cfc11_sat[0][0];
	var[14] = &mn_cfc12_sat[0][0];
	var[15] = &mn_sf6_sat[0][0];
#endif
