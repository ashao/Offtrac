...
#ifdef AR_N2_O2
Gas_const_t ar_props,n2_props,o2_props;
double ***mn_ar;
double ***mn_n2;
double ***mn_o2;
double ***ar;
...
#ifdef AR_N2_O2
	flags[10] = 1; rflags[10]=1; // Argon tracer
	flags[11] = 1; rflags[11]=1; // Nitrogen tracer
	flags[12] = 1; rflags[12]=1; // Oxygen tracer
#endif
...
if (flags[12])
	set_darray3d_zero(mn_o2,NZ,NXMEM,NYMEM);
#endif
...
if (usehindcast) {
	...
	read_gas_exchange_fields(directory, isnxt);
}
...
#ifdef AR_N2_O2
		if (flags[10])
			add_darray3d(mn_ar,ar,NZ,NXMEM,NYMEM);
		if (flags[11])
			add_darray3d(mn_n2,n2,NZ,NXMEM,NYMEM);
		if (flags[12])
			add_darray3d(mn_o2,o2,NZ,NXMEM,NYMEM);
#endif
...
#ifdef AR_N2_O2
			if (flags[10])
				mult_darray3d(mn_ar, NZ, NXMEM, NYMEM, frac);
			if (flags[11])
				mult_darray3d(mn_n2, NZ, NXMEM, NYMEM, frac);
			if (flags[12])
				mult_darray3d(mn_o2, NZ, NXMEM, NYMEM, frac);
#endif
...
#endif
#ifdef AR_N2_O2
copy_darray3d(mn_ar,ar,NZ,NXMEM,NYMEM);
copy_darray3d(mn_n2,n2,NZ,NXMEM,NYMEM);
copy_darray3d(mn_o2,o2,NZ,NXMEM,NYMEM);
#endif
