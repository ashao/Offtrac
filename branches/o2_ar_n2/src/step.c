...
#ifdef AR_N2_O2
extern Gas_const_t ar_props,n2_props,o2_props;
extern double ***mn_ar;
extern double ***mn_n2;
extern double ***mn_o2;
extern double ***ar;
...
# endif
	...
	copy_from_main_tracer_array(ar,ar_props.tridx,tr);
	copy_from_main_tracer_array(n2,n2_props.tridx,tr);
	copy_from_main_tracer_array(o2,o2_props.tridx,tr);
	...
	for (i = 0; i < NXMEM; i++)
		for (j = 0; j < NYMEM; j++) {
			...
			ar[0][i][j]=gas_exchange_const( ar_props, i, j, ar[0][i][j], mldepth_start,
					mldepth_end, dt_gasex, ndt_gasex );
			n2[0][i][j]=gas_exchange_const( n2_props,i,j, n2[0][i][j], mldepth_start,
					mldepth_end, dt_gasex, ndt_gasex );
			o2[0][i][j]=gas_exchange_const( o2_props,i,j, o2[0][i][j], mldepth_start,
					mldepth_end, dt_gasex, ndt_gasex );
			ar[1][i][j]=ar[0][i][j];
			n2[1][i][j]=n2[0][i][j];
			o2[1][i][j]=o2[0][i][j];
	...
	printf("Argon: %f Nitrogen: %f Oxygen: %f\n",ar[0][100][100],n2[0][100][100],o2[0][100][100]);
	...
	copy_to_main_tracer_array(ar,ar_props.tridx,tr);
	copy_to_main_tracer_array(n2,n2_props.tridx,tr);
	copy_to_main_tracer_array(o2,o2_props.tridx,tr);
