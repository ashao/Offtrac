...
//HF
	...
	close_file(&cdfid,&file);
/* ashao: Doesn't seem to be used 04.13.2012
...
void read_hind( int cmon )
	...
	imon=cmon%NMONTHSCLIM;
	inxt=(cmon+1)%NMONTHSCLIM;
	if (imon==0) iprv=NMONTHSCLIM-1;
	else iprv=imon-1;

    }

    printf("READING STUFF");
    tmp3d = alloc3d_f(NZ+1,NXMEM,NYMEM);
    printf("Loading midx %d...",imon);
    printf("uh...");
    read_var3d(uhfile,"uh",inxt,tmp3d,NZ);
    for (k=0;k<NZ;k++)
        for (i=0;i<NXTOT;i++)
            for (j=0;j<NYTOT;j++)
                uhtm[k][i+2][j+2]= tmp3d[k][i][j];
    printf("vh...");
    read_var3d(vhfile,"vh",inxt,tmp3d,NZ);
    for (k=0;k<NZ;k++)
        for (i=0;i<NXTOT;i++)
            for (j=0;j<NYTOT;j++)
                vhtm[k][i+2][j+2]= tmp3d[k][i][j];
    printf("wd...\n");
    read_var3d(wdfile,"wd",inxt,tmp3d,NZ+1);
    for (k=0;k<NZ+1;k++)
        for (i=0;i<NXTOT;i++)
            for (j=0;j<NYTOT;j++)
                wd[k][i+2][j+2]= dt * tmp3d[k][i][j];

    printf("h...\n");
    read_var3d(hfile,"h",imon,tmp3d,NZ);
    for (k=0;k<NZ;k++) {
        for (i=0;i<NXTOT;i++) {
            for (j=0;j<NYTOT;j++) {
                h[k][i+2][j+2]= tmp3d[k][i][j];
                hstart[k][i+2][j+2]= tmp3d[k][i][j];
            }
        }
    }
    read_var3d(hfile,"h",inxt,tmp3d,NZ);
    for (k=0;k<NZ;k++)
        for (i=0;i<NXTOT;i++)
            for (j=0;j<NYTOT;j++)
                hend[k][i+2][j+2]= tmp3d[k][i][j];

    read_var3d(tsfile,"temp",inxt,tmp3d,NZ);
    for (k=0;k<NZ;k++)
        for (i=0;i<NXTOT;i++)
            for (j=0;j<NYTOT;j++)
                Temptm[k][i+2][j+2]= tmp3d[k][i][j];

    read_var3d(tsfile,"salt",inxt,tmp3d,NZ);
    for (k=0;k<NZ;k++)
        for (i=0;i<NXTOT;i++)
            for (j=0;j<NYTOT;j++)
                Salttm[k][i+2][j+2]= tmp3d[k][i][j];
    ...
    printf("Example read in data:\n");
    printf("imon, inxt, iprv: %d %d %d\n",imon,inxt,iprv);
    printf("UH=%f VH=%f WD=%f H=%f T=%f S=%f\n",uhtm[0][100][100], vhtm[0][100][100],
	wd[0][100][100],h[0][100][100],Temptm[0][100][100],Salttm[0][100][100]);


    }

void read_var3d( char inpath[200], char varname[200], int imon, float ***data, int numz)
    {
    ...
    tmp  = alloc3d_f(numz,NYTOT,NXTOT);
    if ((status = nc_get_vara_float(cdfid,varid,start,count,tmp[0][0])))
	ERR(status);
    
    for (k=0;k<numz-1;k++)
	for (i=0;i<NXTOT;i++)
	    for (j=0;j<NYTOT;j++) {		
		data[k][i][j]= tmp[k][j][i];
    }
    free(tmp);
    }
